...
def plot_predictions(y_true, y_pred, target_name, r2, mae, rmse, model_name=""):
    ...
    plt.tight_layout()
    filename = f"{target_name.lower().replace(' ', '_')}_{model_name.lower().replace(' ', '_')}_final.png"
...
def train_and_evaluate_target(X, y, target_name, other_y=None, random_state=42):
    """Train and evaluate with advanced optimization"""
    # Scale features
    ...
    if other_y is not None:
        ...
        print(f"\n  Train: {len(X_scaled)}, Features: {X_scaled.shape[1]} (including auxiliary target)")
    else:
        print(f"\n  Train: {len(X_scaled)}, Features: {X_scaled.shape[1]}")
    ...
    # 1. Advanced RandomForest with feature selection
    print(f"\n  1. Training Optimized RandomForest for {target_name}...")
    
    # Feature selection
    selector = SelectFromModel(RandomForestRegressor(n_estimators=200, random_state=random_state), 
                               max_features=800, threshold=-np.inf)
    ...
    print(f"\n  2. Training Optimized Gradient Boosting for {target_name}...")
    gb_params = {
    ...
    estimators = [
        ('rf', RandomForestRegressor(n_estimators=1000, max_depth=25, random_state=random_state, n_jobs=-1)),
    ...
    results['Stacking'] = {
        ...
        'best_params': 'N/A',
        'predictions': (y_test, y_pred_stack)
    }
    ...
    # Plot best model
    y_true, y_pred = best_result['predictions']
    plot_predictions(y_true, y_pred, target_name, 
                    best_result['test_r2'], best_result['mae'], best_result['rmse'], 
                    best_model_name)
...
def main():
    """Main function"""
    print("=" * 70)
    print("Final Best Model - Target: R² > 0.7 for both Yield and ee")
    print("=" * 70)
    ...
    # First pass: train yield without ee
    print("\n" + "=" * 70)
    print("PASS 1: Training YIELD (without ee)")
    print("=" * 70)
    yield_results_1, yield_best_1 = train_and_evaluate_target(X, y_yield, "Yield", other_y=None)
    
    # First pass: train ee without yield
    print("\n" + "=" * 70)
    print("PASS 1: Training ENANTIOMERIC EXCESS (without yield)")
    print("=" * 70)
    ee_results_1, ee_best_1 = train_and_evaluate_target(X, y_ee, "Enantiomeric Excess", other_y=None)
    
    # Second pass: train yield with ee as auxiliary feature
    print("\n" + "=" * 70)
    print("PASS 2: Training YIELD (with ee as auxiliary feature)")
    print("=" * 70)
    yield_results_2, yield_best_2 = train_and_evaluate_target(X, y_yield, "Yield", other_y=y_ee)
    
    # Second pass: train ee with yield as auxiliary feature
    print("\n" + "=" * 70)
    print("PASS 2: Training ENANTIOMERIC EXCESS (with yield as auxiliary feature)")
    print("=" * 70)
    ee_results_2, ee_best_2 = train_and_evaluate_target(X, y_ee, "Enantiomeric Excess", other_y=y_yield)
    ...
    print("-" * 70)
    print("\nWithout auxiliary feature:")
    for name, res in yield_results_1.items():
    ...
    print("\nWith ee as auxiliary feature:")
    for name, res in yield_results_2.items():
        marker = " ✓" if res['cv_r2'] >= 0.7 else (" ○" if res['cv_r2'] >= 0.6 else "")
        print(f"  {name:20s}: CV R² = {res['cv_r2']:.4f} (+/- {res['cv_std']:.4f}), Test R² = {res['test_r2']:.4f}{marker}")
    
    print("\n" + "-" * 70)
    print("ENANTIOMERIC EXCESS PREDICTION RESULTS:")
    print("-" * 70)
    print("\nWithout auxiliary feature:")
    for name, res in ee_results_1.items():
        marker = " ✓" if res['cv_r2'] >= 0.7 else (" ○" if res['cv_r2'] >= 0.6 else "")
        print(f"  {name:20s}: CV R² = {res['cv_r2']:.4f} (+/- {res['cv_std']:.4f}), Test R² = {res['test_r2']:.4f}{marker}")
    
    print("\nWith yield as auxiliary feature:")
    for name, res in ee_results_2.items():
        marker = " ✓" if res['cv_r2'] >= 0.7 else (" ○" if res['cv_r2'] >= 0.6 else "")
        print(f"  {name:20s}: CV R² = {res['cv_r2']:.4f} (+/- {res['cv_std']:.4f}), Test R² = {res['test_r2']:.4f}{marker}")
    
    # Best overall results
    best_yield = max([yield_results_1, yield_results_2], key=lambda x: max(r['cv_r2'] for r in x.values()))
    best_ee = max([ee_results_1, ee_results_2], key=lambda x: max(r['cv_r2'] for r in x.values()))
    
    best_yield_model = max(best_yield.keys(), key=lambda k: best_yield[k]['cv_r2'])
    best_ee_model = max(best_ee.keys(), key=lambda k: best_ee[k]['cv_r2'])
    
    yield_cv_r2 = best_yield[best_yield_model]['cv_r2']
    ee_cv_r2 = best_ee[best_ee_model]['cv_r2']
    
    print(f"\n{'='*70}")
    print("BEST OVERALL RESULTS:")
    print(f"  Yield: {best_yield_model} - CV R² = {yield_cv_r2:.4f}")
    print(f"  ee:    {best_ee_model} - CV R² = {ee_cv_r2:.4f}")
    
    if yield_cv_r2 >= 0.7 and ee_cv_r2 >= 0.7:
        print("\n  🎉 BOTH TARGETS ACHIEVED CV R² >= 0.7!")
    elif yield_cv_r2 >= 0.7:
        print(f"\n  ⚠️  Only Yield achieved CV R² >= 0.7")
        print(f"     ee gap: {0.7 - ee_cv_r2:.4f}")
    elif ee_cv_r2 >= 0.7:
        print(f"\n  ⚠️  Only ee achieved CV R² >= 0.7")
        print(f"     Yield gap: {0.7 - yield_cv_r2:.4f}")
    else:
        print(f"\n  ❌ Neither target achieved CV R² >= 0.7")
        print(f"     Yield gap: {0.7 - yield_cv_r2:.4f}")
        print(f"     ee gap:    {0.7 - ee_cv_r2:.4f}")
