"""
Optimized Random Baseline Benchmark Script for suzuki_HTE
...
def generate_desc_file(df_ground, desc_dfs, desc_columns):
    ...
    for col in existing_cols:
        desc_table = desc_dfs[col].copy()  # add copy to prevent warnings from subsequent operations
        desc_table = desc_table.select_dtypes(include=["float64", "float32"])

        desc_table = desc_table.add_prefix(f"{col}_")
...
def demo_random_benchmark(dataset="HTE_datasets/suzuki_HTE/suzuki_HTE.csv", desc_columns=["base", "ligand", "solvent"], num_seeds=10):
    ...
    for round_id in range(1, num_seeds + 1):
        ...
        final_Conversion = df_round.iloc[-1]["Conversion_best"]
        print(f"  Final Best Yield: {final_Conversion:.2f}")
    ...
    if all_results:
        ...
        agg_dict = {"Conversion_best": ["mean", "std"], "n_experiments": "mean"}
        df_mean = df_merged.groupby("step").agg(agg_dict).reset_index()
        # Flatten multi-level column names
        df_mean.columns = ["step", "Conversion_best_mean", "Conversion_best_std", "n_experiments_mean"]
        ...
        final_Conversions = df_merged.groupby("round_id")["Conversion_best"].last()
        ...
        with open(timing_filename, "w") as f:
            ...
            f.write("-" * 80 + "\nPerformance Summary:\n" + "-" * 80 + "\n")
            f.write(f"Final Yield_best: Mean={final_Conversions.mean():.2f}, Std={final_Conversions.std():.2f}\n")
            f.write("-" * 80 + "\nPer Round:\n" + "-" * 80 + "\n")
            f.write(f"{'Round':<8} {'Seed':<8} {'Yield_best':<15} {'Cost_best':<15}\n")
            for round_id in range(1, num_seeds + 1):
                f.write(f"{round_id:<8} {round_id:<8} {final_Conversions[round_id]:<15.2f}\n")
            f.write("\n" + "=" * 80 + "\n")
