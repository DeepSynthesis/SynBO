...
def generate_enhanced_features(df, use_radius_3=True):
    """Generate enhanced features using RDKit fingerprints with multiple radii"""
    feature_list = []
    ...
    for smiles_col in smiles_cols:
        ...
        morgan_fps_r2 = []
        # Morgan fingerprints with radius 3 (if enabled)
        morgan_fps_r3 = [] if use_radius_3 else None
        maccs_fps = []
        rdkit_fps = []
        descriptors = []
        ...
        for smiles in df[smiles_col]:
            # Morgan radius 2
            morgan_r2 = smiles_to_fingerprints(smiles, radius=2, n_bits=2048)
            morgan_fps_r2.append(morgan_r2)
            
            # Morgan radius 3
            if use_radius_3:
                morgan_r3 = smiles_to_fingerprints(smiles, radius=3, n_bits=2048)
                morgan_fps_r3.append(morgan_r3)
        ...
        morgan_fps_r2 = np.array(morgan_fps_r2)
        maccs_fps = np.array(maccs_fps)
        rdkit_fps = np.array(rdkit_fps)
        descriptors = np.array(descriptors)
        
        # Apply PCA
        n_comp_morgan = min(80, morgan_fps_r2.shape[1])
        n_comp_rdkit = min(80, rdkit_fps.shape[1])
        n_comp_desc = min(40, descriptors.shape[1])
        ...
        morgan_reduced = pca_morgan.fit_transform(morgan_fps_r2)
        rdkit_reduced = pca_rdkit.fit_transform(rdkit_fps)
        desc_reduced = pca_desc.fit_transform(descriptors)
        
        # Create DataFrames
        morgan_cols = [f"{smiles_col}_morgan2_{i}" for i in range(n_comp_morgan)]
...
def train_and_evaluate_optimized(X, y, target_name, random_state=42):
    """Train and evaluate with multiple optimized models"""
    # Scale features
    ...
    # 1. Optimized RandomForest
    print("\n  1. Training Optimized RandomForest...")
    rf_params = {
    ...
    cv_scores_rf = cross_val_score(rf_best, X_train, y_train, cv=kf, scoring='r2')
    ...
    cv_scores_gb = cross_val_score(gb_best, X_train, y_train, cv=kf, scoring='r2')
    ...
    estimators = [
        ('rf', RandomForestRegressor(n_estimators=500, max_depth=30, random_state=random_state, n_jobs=-1)),
    ...
    cv_scores_stack = cross_val_score(stacking, X_train, y_train, cv=kf, scoring='r2')
...
def main():
    """Main function"""
    print("=" * 70)
    print("Final Optimized Modeling with Grid Search and Ensemble")
    print("=" * 70)
    ...
    X = generate_enhanced_features(df, use_radius_3=False)
    ...
    print("=" * 70)
    y_yield = df["yield"]
    yield_results, yield_best = train_and_evaluate_optimized(X, y_yield, "Yield")
    ...
    print("=" * 70)
    y_ee = df["ee"]
    ee_results, ee_best = train_and_evaluate_optimized(X, y_ee, "Enantiomeric Excess")
