...
def extract_metrics(metrics_summary):
    ...
    # AUC_HC: Area Under Curve - Hypervolume
    ...
    metrics['AUC_HC'] = f"{auc_hc_avg:.6f}±{auc_hc_std:.6f}"
    
    # Additional yield and cost metrics for reference
    aot_yield = metrics_summary.get('average_optimal_targets', {}).get('yield', {})
    metrics['AOT_yield'] = f"{aot_yield.get('average', 0):.4f}±{aot_yield.get('std', 0):.4f}"
    
    aot_cost = metrics_summary.get('average_optimal_targets', {}).get('cost', {})
    metrics['AOT_cost'] = f"{aot_cost.get('average', 0):.6f}±{aot_cost.get('std', 0):.6f}"
    
    auc_yield = metrics_summary.get('auc_of_optimization', {}).get('yield', {})
    metrics['AUC_yield'] = f"{auc_yield.get('average', 0):.4f}±{auc_yield.get('std', 0):.4f}"
    
    auc_cost = metrics_summary.get('auc_of_optimization', {}).get('cost', {})
    metrics['AUC_cost'] = f"{auc_cost.get('average', 0):.6f}±{auc_cost.get('std', 0):.6f}"
...
def aggregate_results(results_dir='benchmark/results', output_file='benchmark/benchmark_results_summary.csv'):
    ...
    fieldnames = [
        'dataset', 'experiment_name', 'base_seed', 'num_rounds', 'iterations', 'batch_size',
        'opt_metrics', 'opt_directions', 'opt_type', 'desc_normalize', 'sampling_method',
        'refine_desc', 'optimize_method', 'surrogate_model', 'acq_func',
        'AOT_HV', 'AUC_HC', 'AOT_yield', 'AOT_cost', 'AUC_yield', 'AUC_cost'
    ]
