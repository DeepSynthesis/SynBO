...
def find_existing_experiment(base_dir_str, current_config):
    """Scan all folders in base_dir to find existing config.json"""
...
def run_simulation(experiment_dir, desc_dict, condition_dict):
    ...
    for round_idx in range(CONFIG["num_rounds"]):
        ...
        print(f"Optimization Goal: {opt_direct}imize {target_metric} until >= {target_threshold} (Max Iter: {max_iterations})")
        ...
        for i in range(max_iterations):
            ...
            if target_metric in current_batch_df.columns:
                if opt_direct == "max":
                    batch_best = current_batch_df[target_metric].max()
                    best_value_so_far = max(best_value_so_far, batch_best)
                    if best_value_so_far >= target_threshold:
                        threshold_met = True
                elif opt_direct == "min":
                    batch_best = current_batch_df[target_metric].min()
                    best_value_so_far = min(best_value_so_far, batch_best)
                    if best_value_so_far <= target_threshold:
                        threshold_met = True

            print(f">>> Best {target_metric} so far: {best_value_so_far:.2f} (Target: {target_threshold})")

            if threshold_met:
                print(f"🎉 SUCCESS! Target threshold reached at Iteration {i}. Stopping current round.")
                break

        if not threshold_met:
            print(f"⚠️ Reached maximum iterations ({max_iterations}) without meeting target threshold ({target_threshold}).")
