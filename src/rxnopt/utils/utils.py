...
def normalize_data(
    ...
    try:
        ...
        console.print(f"[red]Normalization error: {str(e)}[/red]")
        console.print("[yellow]Returning zero array as fallback[/yellow]")
...
def array_process(
    ...
    """
    desc_arrs = [
        [desc_dict[k].loc[name].values for name in condition_dict[k]] 
    ...
    console.print(f"[green]Generated {len(total_desc_arr):,} total combinations[/green]")
...
def done_array_process(
    ...
    if len(matches) != len(prev_rxn_list):
        raise AssertionError(
            f"Number of matches ({len(matches)}) does not match "
            f"number of reactions ({len(prev_rxn_list)})"
        )
    
    matches = np.array(matches).squeeze()
    console.print(f"[green]Matched {len(matches)} completed reactions[/green]")
