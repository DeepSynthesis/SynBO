...
def example_current_hv():
    ...
    condition_dict = {
        "solvent": ["toluene", "DMF", "THF"],
        "base": ["NaOH", "K2CO3", "Et3N"],
        "temperature": [25, 50, 75]
    }
    rxn_opt.load_rxn_space(condition_dict)
    ...
    rxn_opt.load_prev_rxn(prev_data)
    
    # Calculate hypervolume for current state (all batches)
    hv_result = rxn_opt.calculate_current_hv()
    print("\n=== Current Hypervolume ===")
    ...
    hv_batch0 = rxn_opt.calculate_current_hv(batch_id=0)
...
def example_hv_by_batch():
    ...
    rxn_opt = ReactionOptimizer(
        ...
        opt_metric_settings=[
            {"opt_direct": "max", "opt_range": [0, 100]},
            {"opt_direct": "min", "opt_range": [0, 1000]}
        ],
        quiet=False
    )
    ...
    hv_by_batch = rxn_opt.calculate_hv_by_batch()
...
def example_direct_function_call():
    ...
    # Create sample data
    prev_data = pd.DataFrame({
        "yield": [45.2, 67.8, 52.3, 71.5, 78.2],
        "ee": [78.5, 85.2, 72.1, 88.7, 82.3],
        "batch": [0, 0, 0, 1, 1]
    })
    ...
    # Calculate by batch
    hv_by_batch = calculate_hypervolume_by_batch(
        prev_rxn_info=prev_data,
        opt_metrics=opt_metrics,
        opt_metric_settings=opt_metric_settings
    )
