...
def train_and_evaluate_ee(X, y_ee, y_yield, random_state=42):
    """Train and evaluate ee prediction with feature selection and yield as auxiliary feature"""
    # Scale features
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    
    # Add yield as a feature (multi-task learning approach)
    scaler_yield = StandardScaler()
    yield_scaled = scaler_yield.fit_transform(y_yield.values.reshape(-1, 1))
    
    # Combine features
    X_with_yield = np.hstack([X_scaled, yield_scaled])
    
    # Split data
    X_train, X_test, y_train, y_test, yield_train, yield_test = train_test_split(
        X_with_yield, y_ee, y_yield, test_size=0.15, random_state=random_state, shuffle=True
    )
    
    print(f"\n  Train: {len(X_train)}, Test: {len(X_test)}")
    print(f"  Features: {X_with_yield.shape[1]} (including yield)")
    ...
    print(f"\n  Best Model for ee: {best_model_name}")
...
def main():
    """Main function"""
    print("=" * 70)
    print("Optimized ee Modeling with Feature Selection")
    print("=" * 70)
    ...
    # Predict ee with optimizations
    print("\n" + "=" * 70)
    print("Predicting ENANTIOMERIC EXCESS (ee) with Optimizations")
    print("=" * 70)
    ee_results, ee_best = train_and_evaluate_ee(X, y_ee, y_yield)
    ...
    print(f"\n{'='*70}")
    print(f"BEST ee CV R²: {ee_cv_r2:.4f} (using {ee_best})")
    
    if ee_cv_r2 >= 0.6:
        print("\n  ✅ ee ACHIEVED CV R² >= 0.6!")
    else:
        print(f"\n  ❌ ee did not achieve CV R² >= 0.6")
        print(f"     Gap: {0.6 - ee_cv_r2:.4f}")
