...
def calculate_expected_extreme(df, col_name, n=50, extreme_type="max", num_simulations=10000):
    ...
    # Compute total combinations C(N, n)
    total_combinations = math.comb(N, n)

    # Use Python big integers for accumulation to avoid float precision loss
    numerator = 0

    if extreme_type == "max":
        # If the i-th element (sorted) is the sample maximum
        # Must select this element, remaining n-1 elements from the i smaller elements
        # Therefore combinations = C(i, n-1)
        # Valid indices from n-1 to N-1
        for i in range(n - 1, N):
            ways = math.comb(i, n - 1)
            numerator += sorted_data[i] * ways

    elif extreme_type == "min":
        # If the i-th element (sorted) is the sample minimum
        # Must select this element, remaining n-1 elements from the N-1-i larger elements
        # Therefore combinations = C(N-1-i, n-1)
        # Valid indices from 0 to N-n
        for i in range(N - n + 1):
            ways = math.comb(N - 1 - i, n - 1)
            numerator += sorted_data[i] * ways

    # Use Decimal for high-precision division to prevent overflow with large N
    getcontext().prec = 50
...
if __name__ == "__main__":
    ...
    print(f"Exact: {exact_max:.4f}")
    print(f"Error rate: {abs(sim_max - exact_max) / exact_max * 100:.4f}%\n")
    ...
    print(f"Exact: {exact_min:.4f}")
    print(f"Error rate: {abs(sim_min - exact_min) / abs(exact_min) * 100:.4f}%")
